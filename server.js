...
app.post("/recommendations", async (req, res) => {
  if(!req.body) {
    return res.status(400).send({ message: "Bad Request - must send a JSON body with three artists" })
  }
  
  const { artist1, artist2, artist3 } = req.body
  
  if(!artist1 || !artist2 || !artist3) {
    return res.status(400).send({ message: "Bad Request - must pass three artists" })
  }
  ...
  try {
    ...
    if(!result1.artists || !result1.artists.items || !result1.artists.items.length ) {
    ...
    if(!result2.artists || !result2.artists.items || !result2.artists.items.length ) {
    ...
    if(!result3.artists || !result3.artists.items || !result3.artists.items.length ) {
      return res.status(404).send({ message: `Artist ${artist1} not found.` })
    }
